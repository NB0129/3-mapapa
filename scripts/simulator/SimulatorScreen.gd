...
func _rebuild_result_list() -> void:
	...
	for i in range(_results.size()):
		...
		row_panel.custom_minimum_size = Vector2(LEFT_W - 16, RESULT_ROW_H)
		var s := StyleBoxFlat.new()
		if is_best:
		...
		tile_img.position = Vector2(32, 4)
		tile_img.size = Vector2(46, 64)
		...
		# 牌名
		var name_lbl := _make_label(str(r.get("tile_name", "")), Vector2(84, 6), 20)
		name_lbl.size = Vector2(160, 46)
		name_lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		row_panel.add_child(name_lbl)

		# 向聴テキスト
		var shan := int(r.get("shanten", 99))
		var shan_lbl := _make_label(str(r.get("shanten_text", "")), Vector2(250, 8), 20)
		shan_lbl.add_theme_color_override("font_color", Color(0.35, 1.0, 0.45) if shan == 0 else Color(0.9, 0.9, 0.7))
		row_panel.add_child(shan_lbl)

		# 有効牌枚数
		var eff_lbl := _make_label("有効牌 %d枚（期待%d枚）" % [r.get("effective_count", 0), r.get("effective_count_expected", 0)], Vector2(400, 8), 17)
		...
		if tenpai_r < 0.0:
			var dash := _make_label("—", Vector2(84, 58), 17)
			dash.add_theme_color_override("font_color", Color(0.45, 0.45, 0.45))
			row_panel.add_child(dash)
		elif shanten_val == 0:
			var a_lbl := _make_label("和了率 %.2f%%" % (agari_r * 100.0), Vector2(84, 58), 17)
			a_lbl.add_theme_color_override("font_color", Color(1.0, 0.88, 0.4))
			row_panel.add_child(a_lbl)
		elif shanten_val == 1:
			var t_lbl := _make_label("テンパイ率 %.2f%%" % (tenpai_r * 100.0), Vector2(84, 58), 17)
			t_lbl.add_theme_color_override("font_color", Color(0.55, 0.88, 1.0))
			row_panel.add_child(t_lbl)
			var a_lbl := _make_label("和了率 %.2f%%" % (agari_r * 100.0), Vector2(276, 58), 17)
			a_lbl.add_theme_color_override("font_color", Color(1.0, 0.88, 0.4))
			row_panel.add_child(a_lbl)
		elif shanten_val == 2:
			var t_lbl := _make_label("1シャンテン率 %.2f%%" % (tenpai_r * 100.0), Vector2(84, 58), 16)
			t_lbl.add_theme_color_override("font_color", Color(0.55, 0.88, 1.0))
			row_panel.add_child(t_lbl)
			if agari_r >= 0.0:
				var a_lbl := _make_label("テンパイ率 %.2f%%" % (agari_r * 100.0), Vector2(284, 58), 16)
				a_lbl.add_theme_color_override("font_color", Color(1.0, 0.88, 0.4))
				row_panel.add_child(a_lbl)
		else:
			var t_lbl := _make_label("1シャンテン率 %.2f%%" % (tenpai_r * 100.0), Vector2(84, 58), 17)
			t_lbl.add_theme_color_override("font_color", Color(0.55, 0.88, 1.0))
			row_panel.add_child(t_lbl)

		if is_actual:
			var act_tag := _make_label("←実際", Vector2(580, 8), 18)
		...
		for game_id: int in eff:
			var dummy := {"id": game_id, "is_red": false, "is_gold": false, "is_haku_pochi": false}
			var tex := _get_tile_texture(dummy)
			if tex:
				var ti := TextureRect.new()
				ti.position = Vector2(eff_x, eff_row_y)
				ti.size = Vector2(EFF_IMG_W, EFF_IMG_H)
				...
				eff_x += EFF_IMG_W + 2
			var cnt_lbl := _make_label("×%d" % eff[game_id], Vector2(eff_x, eff_row_y + 14), 15)
		...
		var bd_breakdown: Dictionary = r.get("tile_breakdown", {})
		if not bd_breakdown.is_empty() and shanten_val <= 2:
			eff_row_y += EFF_IMG_H + 8
			for gid: int in bd_breakdown:
				var bd: Dictionary = bd_breakdown[gid]
				...
				var bd_lbl := _make_label(bd_txt, Vector2(84, eff_row_y), 13)
				bd_lbl.add_theme_color_override("font_color", Color(0.70, 0.82, 0.70))
				row_panel.add_child(bd_lbl)
				eff_row_y += 18

		var row_h := maxf(RESULT_ROW_H, eff_row_y + EFF_IMG_H + 8) if not eff.is_empty() else RESULT_ROW_H
